/* Call header */ {
    ...
    function getCurMenuItemActive(entries) {
        entries.forEach((entry)=>{
            if (entry.isIntersecting) menuItemsEl[Number(entry.target.dataset.page)].classList.add("active");
            else menuItemsEl[Number(entry.target.dataset.page)].classList.remove("active");
        });
    }
    ...
    function getImgPosition(i2) {
        if (i2 < middleImgIndex - 1) {
            const posImgWithoutMainClasses = 40 - (middleImgIndex - i2) * 15;
            imgBoxNodeListEl[arrImgNumbers[i2]].style.left = posImgWithoutMainClasses + "%";
        }
        if (i2 >= middleImgIndex - 1 && i2 <= middleImgIndex + 1) imgBoxNodeListEl[arrImgNumbers[i2]].style.left = "";
        if (i2 > middleImgIndex + 1) {
            const posImgWithoutMainClasses = 60 - (middleImgIndex - i2) * 15;
            imgBoxNodeListEl[arrImgNumbers[i2]].style.left = posImgWithoutMainClasses + "%";
        }
    }
    for(let i1 = 0; i1 < imgBoxNodeListEl.length; i1 += 1){
        arrImgNumbers.push(i1);
        getImgPosition(i1);
    }
    function getMainClassesAdd() {
        imgBoxNodeListEl[arrImgNumbers[middleImgIndex - 1]].classList.add("left-neighbor");
        imgBoxNodeListEl[arrImgNumbers[middleImgIndex]].classList.add("active");
        imgBoxNodeListEl[arrImgNumbers[middleImgIndex + 1]].classList.add("right-neighbor");
    }
    getMainClassesAdd();
    function getMainClassesRemove() {
        imgBoxNodeListEl[arrImgNumbers[middleImgIndex - 1]].classList.remove("left-neighbor");
        imgBoxNodeListEl[arrImgNumbers[middleImgIndex]].classList.remove("active");
        imgBoxNodeListEl[arrImgNumbers[middleImgIndex + 1]].classList.remove("right-neighbor");
    }
    function getScrollImgRight() {
        getMainClassesRemove();
        imgBoxNodeListEl[arrImgNumbers[0]].classList.add("last");
        imgBoxNodeListEl[arrImgNumbers[imgBoxNodeListEl.length - 1]].classList.remove("last");
        for(let i = 0; i < imgBoxNodeListEl.length; i += 1){
            arrImgNumbers[i] += 1;
            if (arrImgNumbers[i] > imgBoxNodeListEl.length - 1) arrImgNumbers[i] = 0;
            getImgPosition(i);
        }
        getMainClassesAdd();
    }
    function getScrollImgLeft() {
        ...
        imgBoxNodeListEl[arrImgNumbers[imgBoxNodeListEl.length - 1]].classList.add("last");
        for(let i = 0; i < imgBoxNodeListEl.length; i += 1){
            arrImgNumbers[i] -= 1;
            if (arrImgNumbers[i] < 0) arrImgNumbers[i] = imgBoxNodeListEl.length - 1;
            getImgPosition(i);
        }
        getMainClassesAdd();
    }
    function getScrollImg(e) {
        if (e.target.offsetParent.classList.contains("right-neighbor")) getScrollImgRight();
        if (e.target.offsetParent.classList.contains("left-neighbor")) getScrollImgLeft();
    }
    function getScrollImgArrow(e) {
        if (e.code === "ArrowRight") getScrollImgRight();
        if (e.code === "ArrowLeft") getScrollImgLeft();
    }
